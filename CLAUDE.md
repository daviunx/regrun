...
**This repo is GitHub + PUBLIC PyPI, so `gh` is correct here, `glab` cannot work.** It is the one exception to the fleet's NEVER-`gh` rule.
...
- **EVERY schema model forbids extra keys.** A new YAML key must be declared as a field on its model in the same change that starts reading it, or every suite carrying it fails at load. Keys only external orchestration reads (`health_path`, `mcp_health_path`) are declared too, for exactly that reason. Lint rule E006 is the static twin: it validates each parsed file against `TestFile` so the linter can never pass a file `run` refuses to load
...
- **A release is the TAG, not the commit.** `.github/workflows/publish.yml` fires ONLY on `v*.*.*` tags, so pushing `main` publishes NOTHING. A version bump sitting on main is not released
- **The tag push is irreversible.** It lands on public pypi.org/project/regrun, and a burned version can be yanked but NEVER reused. This is the one push in the fleet that stays operator-gated; the `main` push is free
