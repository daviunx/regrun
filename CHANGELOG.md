...
- **`--file <stem-or-glob>`** (repeatable) runs only the matching files, automatically pulling the setup layer and the `requires` closure of each match, in the canonical order. A pattern matching nothing is an error, never a silent zero-file run. Composes with `--group` and `--skip-setup`; a file present only as a dependency keeps all of its groups, because narrowing groups must not drop the variables the selected file needs. **A match in the setup layer pulls every setup file sorting before it** (plus their own closures), because the setup layer is ordered and single-homed: the first setup file owns the suite's `variables:` and `meta.env_file` and every later setup file may read them. A setup file sorting after the match is never pulled, so selecting the first setup file runs it alone. `--skip-setup` still removes the whole layer, which turns a pattern that matched only setup files into an error rather than a zero-file run. Blocking is untouched: a failed setup file still blocks every later file through the setup gate, and the setup prefix is a selection rule, not a graph edge, so shard plans and cycle detection are unchanged.
...
- **One definition of the run order** (`engine/ordering.py`), consumed by the runner, both file selectors, the shard planner and the linter, so a plan, a report and a lint finding cannot disagree about which file comes first. It is a behaviour change against 0.9.x; see **Changed** below before upgrading a pinned suite.
...
- **Lint rule E006** (error) reports a file that parses as YAML but does not validate against the test-file schema: an undeclared key, a missing required key, or a wrong type. The linter reads raw dictionaries, so without this rule it could be blinder than the engine: a file `run` refuses to load reported `0 error(s)` and passed the lint gate. One finding per validation error, carrying the failing location and the message, attributed to the test id where the location names one.
...
- **File run order is now decided in ONE place, and by the STEM rather than the full filename.** `engine/ordering.py` owns the key (layer rank, then the file's stem in byte order) and the runner's file discovery, the dependency graph, `--file`, `--rerun-failed`, the shard planner, the linter's file discovery and its ordering-dependent rules (E002's "sorts after", E005's "runs AFTER it") all consume it. Previously some sites sorted full filenames while others compared bare stems: the two agree on ordinary names and diverge as soon as one stem is a byte-for-byte prefix of another, because the constant `.yaml` suffix then gets compared against a real character. The extension is excluded everywhere now, so **`00_setup` runs before `00_setup-extra`** (a shell's `ls` disagrees, because it compares `.yaml` against `-extra`), `00.b` runs before both, and case is never folded.

  **Upgrading from 0.9.x can reorder files within a layer.** 0.9.x ordered by the full filename, extension included, so the order moves for any pair whose stem is a byte-for-byte prefix of another stem that continues with a character sorting below `.` (a hyphen is the common one: `00_setup` and `00_setup-extra`). Run `--dry-run` and read the file list before upgrading a pinned CI suite. A suite whose names are digits, letters and underscores cannot be affected and keeps the exact order it had.
...
- **Every schema model now rejects unknown keys at load** (`extra="forbid"`), not just `meta`, `assert` and a test. The remaining nine (the file itself, a group, a preflight check, a sweep step, an auth profile, a bash command, `eventually`, `ws_config`, `meta.sql_connection`) took pydantic's default `extra="ignore"`, which silently dropped a misplaced or misspelt key: a top-level `endpoint:` that belongs under `meta:` never reached the runner, a mistyped group flag left the sweep unmarked, a mistyped poll key left the poll on its defaults, and a per-command `assert:` (a test-level key only) was a condition that never ran. The run stayed green and `regrun lint` stayed clean, which is the same silent false-pass family the `assert` tightening closed.

  **Breaking: a suite carrying such a key now fails at load**, at both entrypoints. `run` aborts naming the file, the location and the key, having executed nothing; `lint` reports it as error E006. *No opt-out*, by design: the key was doing nothing, so the fix is to move it to the block that declares it, or to remove it. Reading the reported location tells you which. A suite that carries no undeclared key is unaffected, and every key the schema reference documents still loads unchanged.

  **The abort covers the whole directory.** `--file`, `--rerun-failed` and `--shard` are applied after every discovered file has been parsed and validated, because selection reads what the files declare, so one invalid file aborts every run of that directory, including a `--file` run that did not select it. That is intended: a suite holding a file the engine cannot load is not a suite a narrowed green can be trusted from. Run `regrun lint <dir>` to get the file and key path of every offender at once as E006, rather than one abort at a time.
...
- **Every operator under one JSONPath key is now evaluated.** A condition dict carrying two or more operators (`"$.slug": { not_empty: true, starts_with: "myapp-" }`) previously ran only the first one found in a fixed internal order and dropped the rest with no signal anywhere: the report showed a green test over an assertion that never executed. Each recognised operator now produces its own result, named separately in the report (`json_path($.slug).not_empty`, `json_path($.slug).starts_with`), and the test passes only when all of them pass. A key that is not a recognised operator fails the test instead of being ignored. When at least one match-requiring operator (`equals`, `contains`, `gt`, `gte`, `lt`, `lte`, `starts_with`, `matches`) is present and the path matches nothing, one "Path not found in response" failure still covers all of them rather than one per operator. A condition carrying no recognised operator at all (an empty dict, or only a typo'd key) now reports "Unknown condition type" whether or not the path matched: it previously reported "Path not found in response" when the path missed, which named the wrong cause. Both spellings fail the test, so only the message moves. Single-operator behaviour is unchanged, including `exists`/`not_empty` answering a missing path themselves, `not_contains` passing on an empty match set and `any_contains` failing on one. `eventually` polls treat "all operators pass" as the success condition.

  **Behaviour change: a previously green test can now go red.** Any suite that already carried a second operator under a path has been running only the first one. That second operator now executes for the first time, so it can fail. Such a red is a real assertion finally running, not a regression: read the named operator in the report and fix the product or the expectation. Nothing needs changing in a suite whose paths each carry one operator.
...
- **A test whose evaluated assertion list is EMPTY now FAILS** with an explicit `zero assertions evaluated` error. `assert: {}` or `assert: {json_path: {}}` evaluated zero assertions and passed via `all([]) == True`, so a test could report PASSED having checked nothing.
...
- **Bash child env injection.** `BashRunner` now passes `os.environ` + the resolved `REGRUN_API_ENDPOINT` / `REGRUN_MCP_ENDPOINT` + the run's `RUN_ID` to every command, so bash steps always know the stack under test: `${REGRUN_API_ENDPOINT}` is always correct and hardcoding a host becomes unnecessary rather than merely discouraged.
- **Report provenance.** The report header (and `report.json`) now carries the regrun version that adjudicated the run, the resolved api/mcp endpoints, the run's `RUN_ID` and the lock target, so "why did this go red between tasks" is one line of reading, not archaeology.
- **Lint rules W007–W011:**
  ...
  - **W009**: `json_path` condition whose ONLY operator is `exists: true` (satisfied by null, so use `not_empty` or a value). Inline suppress: `# lint: allow-exists`.
...
- **`preflight:` dependency-health checks.** A top-level `preflight:` block lists read-only probes (a `Test`-shaped body on any runner + a `name` + a `timeout` defaulting to 10s) that run once, before any group, and abort the whole run in seconds naming the failed dependency. This kills the degraded-backend grind regime. Checks are collected across all loaded files in file order; the first failure prints `PREFLIGHT FAILED: <name>` + diagnostics and exits non-zero having executed zero groups. `eventually:` and `capture:` are rejected at validation (a health probe must not retry a degraded backend into looking healthy, nor feed run state). `--skip-preflight` bypasses; `--dry-run` lists the checks; a passing run's report header prints `preflight: N checks passed`.
- **Per-product run lock.** Every run holds an exclusive `fcntl.flock` on `{REGRUN_RUNS_DIR|~/.regrun/runs}/{product}/.lock` for its duration, mechanically enforcing the sweep-first no-concurrency assumption. A second concurrent run for the same product exits code 2 naming the product + lock path. flock self-releases on process death (incl. SIGKILL), so there is no stale-lock protocol. `--no-lock` bypasses. An unusable runs dir degrades to running unlocked (best-effort), never a crash.
