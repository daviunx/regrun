...
- **One definition of the run order** (`engine/ordering.py`): layer rank, then the file's STEM in byte order. The runner's file discovery, the dependency graph, the `--file` and `--rerun-failed` selectors, the shard planner and the linter's ordering-dependent rules (E002's "sorts after", E005's "runs AFTER it") all consume that one key, so a plan, a report and a lint finding cannot disagree about which file comes first. Previously some sites sorted full filenames while others compared bare stems, which agree on ordinary names and diverge as soon as one name is a prefix of another: the constant `.yaml` suffix then gets compared against a real character, and `00_setup-extra.yaml` sorts before `00_setup.yaml` while `00_setup` sorts before `00_setup-extra`. The extension is now excluded everywhere, so **`00_setup` runs before `00_setup-extra`** (a shell's `ls` disagrees), `00.b` runs before both, and case is never folded. A suite whose names are digits, letters and underscores keeps the exact order it had.
