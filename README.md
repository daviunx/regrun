...
**Shard a suite only once it lints W012-clean.** A file that consumes a value another file captured without declaring `meta.requires:` is coupled to that file by run order alone. The full ordered run satisfies it by accident; a shard boundary does not, and the consumer fails on an unresolved variable that reads like a product bug. A DECLARED dependency is safe: the shard unit is a connected component of the `requires` graph, so a file and its closure always land in the same shard. So the fix is to clear the W012 findings first, by producing the value locally, by moving a read-only value shared by several files into the setup layer (setup runs in every shard), or by declaring the dependency. Lint is a static check and cannot see coupling that carries no variable reference, such as a shared fixture-name literal or a count over a shared table, so a clean W012 is the precondition, not the whole proof.
